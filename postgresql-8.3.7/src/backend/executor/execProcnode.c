...
ExecProcNode(PlanState *node)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_IndexScanState:
			...
			IndexScanState * x = (IndexScanState *) node;
			IndexScanDesc scan;
			scan = x->iss_ScanDesc;		
			NameData indexName = scan->indexRelation->rd_am->amname;	
			
			
			if(strcmp(indexName.data ,"hierarchical") == 0){
				if(!(x->estimated)){
					result = NULL;
					break;
				}else{
				...
				printf("PRINTTTTTTTTTINGGGGGGGGGGG %s \n", indexName.data);
				ScanDirection direction;
			 EState     *estate;
			 estate = x->ss.ps.state;
   		         direction = estate->es_direction;
			FmgrInfo   *procedure;
			GET_SCAN_PROCEDURE(amgettuple);
			bool found = DatumGetBool(FunctionCall2(procedure,
											   PointerGetDatum(scan),
											   Int32GetDatum(direction)));
			printf("here middle execindexscan  XXXXXXXXXXX\n");
			//make the tuple slot
			TupleTableSlot *slot;
			slot = x->ss.ss_ScanTupleSlot;	
			HeapTuple	tuple = &(scan->xs_ctup);			
			ExecStoreTuple(tuple,	/* tuple to store */
					   slot,	/* slot to store in */
					   scan->xs_cbuf,		/* buffer containing tuple */
					   false);	/* don't pfree */
			result = slot;
			printf("here after execindexscan  XXXXXXXXXXX\n");				
...
printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!endindescan!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
			...
			if(strcmp(indexName.data ,"hierarchical") == 0){
				printf("ssswwwwwwwwww\n");				 
				ExecClearTuple(x->ss.ps.ps_ResultTupleSlot);
  				 ExecClearTuple(x->ss.ss_ScanTupleSlot);
			}else{
