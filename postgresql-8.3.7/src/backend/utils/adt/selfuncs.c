...
/* 
 * Should be in ~/Research_2/Test/postgresql-8.3.7/src/backend/utils/adt/selfuncs.c
 * The last four pass by reference output parameters can be computed (estimated) using genericcostestimate proc		
 */

Datum
hr_costestimate(PG_FUNCTION_ARGS)
{
	PlannerInfo *root = (PlannerInfo *) PG_GETARG_POINTER(0);
	IndexOptInfo *index = (IndexOptInfo *) PG_GETARG_POINTER(1);
	List	   *indexQuals = (List *) PG_GETARG_POINTER(2);
	...
	*indexCorrelation = 0.0;	//A correlation coefficient is a statistical measure of the degree to which changes to the value of one variable predict change to the value of another (between the index and the original relation), range from 1 to -1
